<?php
function get_category_inclusive($data)
{
    ...
    $acf_inclusive_packages = get_field('inclusive_packages', 'category_' . $category_id);
    $acf_inclusive_packages_data = [];
    if ($acf_inclusive_packages) {
        foreach ($acf_inclusive_packages as $acf_post) {
            $acf_inclusive_packages_data[] = [
                'id'                => $acf_post->ID,
    ...
    return rest_ensure_response($acf_inclusive_packages_data);
}
