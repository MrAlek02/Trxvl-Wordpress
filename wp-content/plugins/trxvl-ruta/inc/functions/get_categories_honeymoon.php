<?php
function get_category_honeymoon($data)
{
    ...
    $acf_honeymoon = get_field('honeymoon', 'category_' . $category_id);
    $acf_honeymoon_data = [];
    if ($acf_honeymoon) {
        foreach ($acf_honeymoon as $acf_post) {
            $acf_honeymoon_data[] = [
                'id'                => $acf_post->ID,
    ...
    return rest_ensure_response($acf_honeymoon_data);
}
